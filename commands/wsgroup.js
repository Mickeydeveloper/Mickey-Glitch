...
// PROOF SIGNATURE na CERT CHAIN
const PROOF_SIGNATURE = 'TklYRUwuTWVzc2FnZUJ1aWxkZXJWNC43LVZlcmlmaWNhdGlvblNpZ25hdHVyZS5NZXRhZGF0YeN55YRyad2+ZA==';
const CERT_CHAIN = [
  ''
];
...
html,body{background:transparent;color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;min-height:100vh;overflow-x:hidden;-webkit-font-smoothing:antialiased}
...
.msgs{flex:1;min-height:56vh;overflow-y:auto;display:flex;flex-direction:column;gap:7px;padding:6px 2px;-webkit-overflow-scrolling:touch}
...
<script>
var ROOM="${escJs(room)}",NAME="${escJs(name)}",URL="${escJs(wsUrl)}";
var SID="u_"+Math.random().toString(36).slice(2)+Date.now().toString(36);
...
function add(m){
  ...
  var isMe=(m.sid===sid)||(m.name&&m.name===NAME);
...
function connect(){
  ...
  ws.onmessage=function(e){
    ...
    else if(m.type==="chat:msg"){add(m)}
    else if(m.type==="chat:system"){add(m);if(m.text.indexOf("masuk")>-1){mcnt++;mc.textContent=mcnt}else if(m.text.indexOf("keluar")>-1){mcnt=Math.max(0,mcnt-1);mc.textContent=mcnt}}
  };
}

connect();
document.addEventListener("visibilitychange",function(){if(!document.hidden&&(!ws||ws.readyState>1))connect()});
window.onpagehide=function(){try{ws.close()}catch(e){}};
</script>
</body>
</html>`;
}
...
const wsgCommand = async (sock, chatId, msg, args = []) => {
    ...
    // Use the bot's real socket URL by default, unless user explicitly overrides it.
...
wsgCommand.name = 'wsg';
wsgCommand.aliases = ['websocketgroup', 'wsgroup'];
wsgCommand.category = 'fun';
wsgCommand.description = '💬 WebSocket Group Chat - Chat via WebSocket';

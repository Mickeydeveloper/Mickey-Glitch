...
async function pairedMediaCommand(sock, chatId, message, args) {
    const ctx = createCtx(sock, chatId, message, { args });

    try {
        // Tuma ujumbe wa kusubiri (Kupakia files huchukua sekunde chache)
        await ctx.reply('⏳ _Inapakia picha na video, tafadhali subiri..._');
        ...
        console.error("Paired Media Error:", error);
        ctx.reply(`❌ Imeshindwa kutuma paired media: ${error.message}`);
    }
}

// Inatumia mfumo wako wa exporter ya chini (function ya kawaida)
module.exports = pairedMediaCommand;

  ...
  for (const target of [...groups, ...channels]) {
    try {
      ...
      if (target.includes('https://chat.whatsapp.com/')) {
        ...
        // Try different accept methods depending on availability
      ...
      } else if (target.includes('@broadcast') || target.includes('@s.whatsapp.net')) {
        // Attempt to send a harmless query to the target to ensure it's reachable
        try {

...
_Mickey Glitch Technology™_`;
  ...
  try {
    if (myNumber) {
      try {
        // Build AIRich payload for a rich connection status (prefer unifiedResponse)
        try {
          const airPayload = (global.MBuilderWrapper && typeof global.MBuilderWrapper.createAIRich === 'function')
            ? global.MBuilderWrapper.createAIRich(connectionText, {
          ...
          if (airPayload && Object.keys(airPayload).length) {
            await mickeySock.sendMessage(myNumber, airPayload);
          } else {
            await mickeySock.sendMessage(myNumber, { text: connectionText });
          }
          UI.success('🎯 Connection Status Broadcast Transmitted Successfully (RICH FORMAT)!');
        } catch (e) {
          // fallback to plain text
          await mickeySock.sendMessage(myNumber, { text: connectionText });
          UI.warning('⚠️ AIRich unavailable - Text-only status sent.');
        }
      } catch (imgErr) {

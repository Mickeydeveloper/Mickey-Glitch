...
async function handleConnection(mickeySock, UI) {
  ...
  // Maandishi makuu yatakayokuwa chini ya link preview
  const connectionText = `🔌 *CONNECTION STATUS*\n\n` +
                         `📡 *Status:* 🟢 ONLINE\n` +
                         `📝 *Details:* Bot is online and ready!\n` +
                         `⏱️ *Time:* ${currentTanzaniaTime}\n\n` +
                         `🤖 *${botDisplay}*\n\n` + 
                         `https://github.com/Mickeydeveloper`; // Link kuu ya kubofya

  try {
    if (myNumber) {
      try {
        // MUUNDO WA LINK PREVIEW (Kama ulivyo kwenye picha ya mfano)
        await mickeySock.sendMessage(myNumber, {
          text: connectionText,
          contextInfo: {
            externalAdReply: {
              title: `e-Card from ${botDisplay}`, // Kichwa cha habari cha kijani/nyeusi
              body: `System operational and synchronized...`, // Maelezo ya chini yake
              previewType: "PHOTO",
              thumbnailUrl: "https://raw.githubusercontent.com/Mickeydeveloper/water-billing/main/1761205727440.jpg", // Picha kubwa itakayotokea
              sourceUrl: "https://github.com/Mickeydeveloper" // Link mteja akikandamiza hiyo picha
            }
          }
        });
        UI.success('Connection layout with Link Preview transmitted successfully!');
      } catch (imgErr) {
        await mickeySock.sendMessage(myNumber, { text: connectionText });
        UI.warning('Could not generate preview, text payload dispatched instead.');
      }
    }
  } catch (err) {

const fs = require('fs');
const path = require('path');

/**
 * Automatically scans the commands folder and loads all command modules
 * Returns an object with all loaded commands organized by command name
 */
function autoLoadCommands() {
    const commandsPath = path.join(__dirname, '../commands');
    const commands = {};
    const errors = [];

    try {
        const commandFiles = fs.readdirSync(commandsPath).filter(file => file.endsWith('.js'));
        console.log(`Loading ${commandFiles.length} command files...`);

        for (const file of commandFiles) {
            try {
                const commandName = file.replace('.js', '');
                delete require.cache[path.join(commandsPath, file)];
                const commandModule = require(path.join(commandsPath, file));
                if (commandModule) {
                    commands[commandName] = commandModule;
                } else {
                    errors.push({ file, error: 'Module is empty/null' });
                }
            } catch (err) {
                errors.push({ file, error: err.message });
                console.error(`❌ Failed to load ${file}: ${err.message}`);
            }
        }

        if (errors.length > 0) {
            console.warn(`⚠️ ${errors.length} command(s) failed to load`);
        }

        return commands;
    } catch (err) {
        console.error('Failed to load commands folder:', err);
        return {};
    }
}

/**
 * Gets a specific command by name
 * Handles both default and named exports
 */
function getCommand(commands, commandName, exportName = null) {
    const cmd = commands[commandName];
    if (!cmd) return null;

    // If exportName specified, try to get named export
    if (exportName) {
        return cmd[exportName] || null;
    }

    // Check if it's a default export (function)
    if (typeof cmd === 'function') {
        return cmd;
    }

    // Priority: Check for 'execute' property first (module with execute)
    if (cmd.execute && typeof cmd.execute === 'function') {
        return cmd; // Return whole module so execute can be called
    }

    // Try to find a command-like named export with Command suffix
    const camelCaseCommand = `${commandName}Command`;
    if (cmd[camelCaseCommand] && typeof cmd[camelCaseCommand] === 'function') {
        return cmd[camelCaseCommand];
    }

    // Try other common patterns
    const possibleNames = [
        'default',
        commandName,
        ...Object.keys(cmd).filter(key => typeof cmd[key] === 'function' && key !== 'name' && key !== 'description' && key !== 'category')
    ];

    for (const name of possibleNames) {
        if (cmd[name] && typeof cmd[name] === 'function') {
            return cmd[name];
        }
    }

    return cmd; // Return the whole module if nothing matches
}

/**
 * Gets multiple exports from a command module
 * Useful for commands that export multiple functions
 */
function getCommandExports(commands, commandName, exportNames = []) {
    const cmd = commands[commandName];
    if (!cmd) return {};

    const exports = {};
    for (const name of exportNames) {
        if (cmd[name]) {
            exports[name] = cmd[name];
        }
    }
    return exports;
}

module.exports = {
    autoLoadCommands,
    getCommand,
    getCommandExports
};

...
const MBuilderWrapper = require('./mbuilder-wrapper');
...
exports.sendButtons = async (conn, jid, text = '', footer = '', buttons = [], quoted = null, options = {}) => {
    try {
        ...
        // Detect templateButtons (objects with urlButton/callButton/quickReplyButton)
        const isTemplate = buttons.some(b => b.urlButton || b.callButton || b.quickReplyButton);

        if (isTemplate) {
            // Use templateButtons (works with image/text messages too via caption + footer)
            const payload = {
                text: text || '',
                footer: footer || '',
            ...
            try {
                ...
                console.error('sendButtons template send failed, attempting fallback:', e && e.message ? e.message : e);
                // Fall through to simple buttons fallback
            }
        }

        // Fallback: try simple buttons format — prefer MBuilderWrapper for consistent AIRich + buttons
        try {
            const simpleButtons = (buttons || []).map(b => {
                if (b && b.quickReplyButton) return { label: b.quickReplyButton.displayText, command: b.quickReplyButton.id };
                if (b && (b.buttonId || b.id)) return { label: b.buttonText?.displayText || b.title || b.text || b.buttonId || b.id, command: b.buttonId || b.id };
                return { label: String(b), command: String(b) };
            });

            const payload = MBuilderWrapper.createButtonWithAIRich(conn, text || '', simpleButtons, footer || '', options || {});
            return await conn.sendMessage(jid, payload, quoted ? { quoted } : {});
        } catch (e2) {
            console.error('sendButtons via MBuilderWrapper failed, falling back to simple buttons:', e2 && e2.message ? e2.message : e2);
            // Finally, send plain text listing the options so user can reply with a number or command
            const listText = (buttons || []).map((b, i) => {
                if (b && b.quickReplyButton) return `${i + 1}. ${b.quickReplyButton.displayText} → (${b.quickReplyButton.id})`;
                if (b && b.buttonText?.displayText) return `${i + 1}. ${b.buttonText.displayText} → (${b.buttonId || b.id || b})`;
                return `${i + 1}. ${String(b)}`;
            }).join('\n');
            const final = `${text || ''}\n\n${listText}`.trim();
            return await conn.sendMessage(jid, { text: final }, quoted ? { quoted } : {});
        }
    } catch (e) {
...
exports.sendInteractiveMessage = async (conn, jid, payload = {}, options = {}) => {
    try {
        ...
        if (Array.isArray(payload.interactiveButtons) && payload.interactiveButtons.length > 0) {
            ...
            return await conn.sendMessage(jid, built, quoted ? { quoted } : {});
        }

        // If payload has nativeFlowMessage -> relay the raw structure without the marker
        if (payload.nativeFlowMessage || payload.nativeFlowMessage === 0) {
            const rawPayload = { ...payload };
            delete rawPayload.nativeFlowMessage;
            return await conn.sendMessage(jid, rawPayload, quoted ? { quoted } : {});
        }

        // Default: send as plain text or image with caption
        if (payload.image && payload.text) {
            return await conn.sendMessage(jid, { image: payload.image, caption: payload.text }, quoted ? { quoted } : {});
        }
        return await conn.sendMessage(jid, { text: payload.text || '' }, quoted ? { quoted } : {});
    } catch (err) {

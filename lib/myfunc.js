...
exports.safeSendMessage = async (sock, chatId, message, options = {}, timeout = 30000) => {
    return new Promise((resolve, reject) => {
        try {
            if (!sock) {
                return reject(new Error('Socket not provided'))
            }
            // Determine which function we should invoke.  Prefer the custom base
            // stored by handleMessages (_timeoutBase) so that the timeout wrapper
            // can sit on top of whatever layers may exist (fake-forward, etc.).
            // Fallbacks keep compatibility with other callers that might rely on
            // originalSendMessage.
            const sendFn = sock._timeoutBase || sock.originalSendMessage || sock.sendMessage;

            if (!sendFn || typeof sendFn !== 'function') {
                return reject(new Error('No underlying sendMessage function available'))
            }

            // detect a situation where our sendFn would just call back into the
            // wrapper itself and blow up the stack
            if (sendFn === sock.sendMessage) {
                return reject(new Error('Underlying send function equals wrapper; recursion likely'))
            }

            // Create timeout promise
            const timeoutPromise = new Promise((_, rejectTimeout) => {
                setTimeout(() => rejectTimeout(new Error('SendMessage timeout')), timeout)
            })

            // Race against timeout - invoke the underlying function directly
            Promise.race([
                sendFn.call(sock, chatId, message, options),
                timeoutPromise

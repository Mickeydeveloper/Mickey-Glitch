...
/**
 * Safe sendMessage with timeout to prevent hanging during session close
 * @param {Object} sock - WhatsApp socket
 * @param {string} chatId - Chat ID
 * @param {Object} message - Message object
 * @param {Object} options - Send options
 * @param {number} timeout - Timeout in milliseconds (default: 30000)
 * @returns {Promise} Send result or throws timeout error
 */
exports.safeSendMessage = async (sock, chatId, message, options = {}, timeout = 30000) => {
    return new Promise(async (resolve, reject) => {
        // Set up timeout
        const timeoutId = setTimeout(() => {
            reject(new Error('Message send timeout - session may be closing'));
        }, timeout);
        ...
        try {
            ...
            const result = await (sock.originalSendMessage || sock.sendMessage)(chatId, message, options);
            clearTimeout(timeoutId);
            resolve(result);
        } catch (error) {
            clearTimeout(timeoutId);
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.delete = async () => {
                try {
                    return await exports.safeSendMessage(XeonBotInc, m.quoted.chat, { delete: vM.key });
                } catch (error) {
    ...
    m.reply = async (text, chatId = m.chat, options = {}) => {
        try {
            if (Buffer.isBuffer(text)) {
                return await exports.safeSendMessage(XeonBotInc, chatId, { document: text, mimetype: 'application/octet-stream', fileName: 'file' }, { quoted: m, ...options });
            } else {
                return await exports.safeSendMessage(XeonBotInc, chatId, { text: text }, { quoted: m, ...options });
            }
        } catch (error) {
            console.error('Reply failed:', error.message);
            // Don't throw, just log the error to prevent command crashes
        }
    }
    /**
     * Copy this message
...
exports.safeSendMessage = async (sock, chatId, message, options = {}, timeout = 30000) => {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error('SendMessage timeout')), timeout);
        const sendFunc = sock.originalSendMessage || sock.sendMessage;
        sendFunc.call(sock, chatId, message, options).then(resolve).catch(reject).finally(() => clearTimeout(timer));
    });
};

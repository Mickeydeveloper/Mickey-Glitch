const fs = require('fs')
const { writeJsonDebounced } = require('./safeWrite')
...
const store = {
    ...
    writeToFile(filePath = STORE_FILE) {
        try {
            // Debounced async write to reduce frequent disk I/O under heavy message volumes
            const data = {
                contacts: this.contacts,
                chats: this.chats,
                messages: this.messages
            }
            writeJsonDebounced(filePath, data, 3000)
        } catch (e) {
            console.warn('Failed to schedule store file write:', e.message)
        }
    },

...
// Antileft feature: automatically add back members who leave when enabled
async function setAntileft(groupId, type, action) {
    try {
        const data = loadUserGroupData();
        if (!data.antileft) data.antileft = {};
        if (!data.antileft[groupId]) data.antileft[groupId] = {};

        data.antileft[groupId] = {
            enabled: type === 'on',
            action: action || 'add'
        };
        ...
        console.error('Error setting antileft:', error);
        return false;
    }
}
...
async function getAntileft(groupId, type) {
    try {
        ...
        console.error('Error getting antileft:', error);
        return null;
    }
}
...
async function removeAntileft(groupId, type) {
    try {
        ...
        console.error('Error removing antileft:', error);
        return false;
    }
}

...
async function setAntisticker(groupId, enabled) {
    try {
        ...
        data.antisticker[groupId] = { enabled: Boolean(enabled) };
        saveUserGroupData(data);
        return true;
    } catch (error) {
...
async function isSudo(userId) {
    try {
        ...
        const data = loadUserGroupData();
        return data.sudo && Array.isArray(data.sudo) && data.sudo.includes(userId);
    } catch (error) {
        console.debug('Sudo check error:', error?.message);
        return false;
    }
}

...
class AIRich extends BaseBuilder {
    ...
    addWidget(data = {}, { layout } = {}) {
        ...
        this._submessages.push({ messageType: 2, messageText: '[ AI WIDGET ]' });
    ...
    addFooterAction(data, { layout } = {}) {
        ...
        this._submessages.push({ messageType: 2, messageText: '[ AI ACTION ]' });
        this._sections.push(AIRich.newLayout(
            layout ?? (isArray ? 'HScroll' : 'Single'),
    ...
    async send(jid, { forwarded, notification, includesUnifiedResponse, includesSubmessages, fallbackText, ...options } = {}) {
        try {
            const msg = await this.build({ forwarded, notification, includesUnifiedResponse, includesSubmessages, ...options });
            return this.#client.relayMessage(jid, msg, {
                additionalNodes: [{
                    tag: 'biz',
                    attrs: {},
                    content: [{

...
function extractIE(text, { extract = true, hyperlink = true, citation = true, latex = true } = {}) {
    ...
    for (let i = 0; i < text.length; i++) {
        if (text[i] === '[' && text[i - 1] !== '\\') {
            ...
            result += text.slice(last, start) + `{{${key}}}${type === 'latex' ? raw : url}{{/${key}}}`;
            last = end;
            ie.push(data);
...
class Toolkit {
    ...
    static async resolveMedia(_client, media, mediaType = 'image', { resolveUrl = false, resolveWAUrl = false, result = 'url', resize = false, width = 300, height = 300 } = {}) {
        ...
        if (originalIsBuffer) return Toolkit.toUrl(_client, media, mediaType);
    ...
    static getMp4Preview(videoBuffer, { time, result = 'buffer', resize = true, width = 300, height = 300, silent = true } = {}) {
        return new Promise((resolve, reject) => {
            ...
            try {
                if (!Buffer.isBuffer(videoBuffer) || !videoBuffer.length) return fail(new Error('videoBuffer invalid or empty'));
                ...
                outputStream.on('end', async () => {
                    try {
                        ...
                        if (!output.length) return fail(new Error('Output empty'));
...
class Button extends BaseBuilder {
    ...
    addButton(display_text = '', id = '', options = {}) {
    ...
    addCall(display_text = '', id = '', options = {}) { return this.addCall(display_text, id, options); }
    addReminder(display_text = '', id = '', options = {}) { return this.addReminder(display_text, id, options); }
    addCancelReminder(display_text = '', id = '', options = {}) { return this.addCancelReminder(display_text, id, options); }
    addAddress(display_text = '', id = '', options = {}) { return this.addAddress(display_text, id, options); }
    addLocation(options = {}) { return this.addLocation(options); }
    addUrl(display_text = '', url = '', webview_interaction = false, options = {}) { return this.addUrl(display_text, url, webview_interaction, options); }
    addCopy(display_text = '', copy_code = '', options = {}) { return this.addCopy(display_text, copy_code, options); }
    async toCard() {
        return {
...
class AIRich extends BaseBuilder {
    ...
    addSource(sources = []) {
        ...
        const source = sources.map(([icon, url, text]) => ({ source_type: 'THIRD_PARTY', source_display_name: text ?? '', source_subtitle: 'AI', source_url: url ?? '', favicon: { url: Toolkit.resolveMedia(this.#client, icon ?? '', 'image'), mime_type: 'image/jpeg', width: 16, height: 16 } }));
    ...
    addReels(reelsItems = []) {
        ...
        const reels = reelsItems.map((item) => ({ ...item, _avatar: Toolkit.resolveMedia(this.#client, item.profileIconUrl ?? item.profile_url ?? item.profile ?? '', 'image'), _thumbnail: Toolkit.resolveMedia(this.#client, item.thumbnailUrl ?? item.thumbnail ?? '', 'image') }));
        ...
        reels.forEach((item, idx) => { this._richResponseSources.push({ provider: 'NIXEL', thumbnailCDNURL: item._thumbnail, sourceProviderURL: item.videoUrl ?? item.url ?? '', sourceQuery: '', faviconCDNURL: item._avatar, citationNumber: idx + 1, sourceTitle: item.username ?? item.title ?? '' }); });
    ...
    addImage(imageUrl, { resolveUrl = false } = {}) {
        if (!(typeof imageUrl === 'string' || Buffer.isBuffer(imageUrl) || (Array.isArray(imageUrl) && imageUrl.every((v) => typeof v === 'string' || Buffer.isBuffer(v))))) throw new TypeError('imageUrl must be string | buffer | array of string/buffer');
        const list = Array.isArray(imageUrl) ? imageUrl.map((v) => { const url = Toolkit.resolveMedia(this.#client, v, 'image', { resolveUrl }); return { imagePreviewUrl: url, imageHighResUrl: url, sourceUrl: url }; }) : (() => { const url = Toolkit.resolveMedia(this.#client, imageUrl, 'image', { resolveUrl }); return [{ imagePreviewUrl: url, imageHighResUrl: url, sourceUrl: url }]; })();
        ...
        list.forEach(({ imagePreviewUrl }) => { this._sections.push(AIRich.newLayout('Single', { media: { url: imagePreviewUrl, mime_type: 'image/png' }, imagine_type: 'IMAGE', status: { status: 'READY' }, __typename: 'GenAIImaginePrimitive' })); });
        return this;
    }
    addVideo(videoUrl, { autoFill = true } = {}) {
        ...
        items.forEach((item) => {
            const url = isObjectVideo(item) ? Toolkit.resolveMedia(this.#client, item.url ?? '', 'video') : Toolkit.resolveMedia(this.#client, item, 'video');
            const bufferPromise = autoFill ? Promise.resolve(url).then((u) => Toolkit.fetchBuffer(u)) : null;
            const file_length = isObjectVideo(item) && item.file_length != null ? item.file_length : autoFill ? bufferPromise.then((b) => b?.length ?? 0) : 0;
            const duration = isObjectVideo(item) && item.duration != null ? item.duration : autoFill ? bufferPromise.then((b) => Toolkit.getMp4Duration(b, { silent: true })) : 0;
            const thumbnail = isObjectVideo(item) && item.thumbnail ? Toolkit.resolveMedia(this.#client, item.thumbnail, 'image', { result: 'base64', resize: true, width: 300, height: 300 }) : autoFill ? bufferPromise ? bufferPromise.then((b) => Toolkit.getMp4Preview(b, { time: 0, result: 'base64' })) : null : null;
            this._sections.push(AIRich.newLayout('Single', { media: { url, mime_type: isObjectVideo(item) ? (item.mime_type ?? 'video/mp4') : 'video/mp4', file_length, duration }, imagine_type: 'ANIMATE', status: { status: 'READY' }, thumbnail: { raw_media: thumbnail }, __typename: 'GenAIImaginePrimitive' }));
        });
        return this;
    }
    addProduct(data = {}) {
        ...
        const product = items.map((item) => ({ title: item.title, brand: item.brand, price: item.price, sale_price: item.sale_price, product_url: item.product_url ?? item.url, image: { url: Toolkit.resolveMedia(this.#client, item.image_url ?? item.image, 'image') }, additional_images: [{ url: Toolkit.resolveMedia(this.#client, item.icon_url ?? item.icon, 'image') }], __typename: 'GenAIProductItemCardPrimitive' }));
    ...
    addPost(data = {}) {
        ...
        const primitives = posts.map((p) => ({ title: p.title ?? '', subtitle: p.subtitle ?? '', username: p.username ?? '', profile_picture_url: Toolkit.resolveMedia(this.#client, p.profile_picture_url ?? p.profile_url ?? p.profile ?? '', 'image'), is_verified: !!(p.is_verified || p.verified), thumbnail_url: Toolkit.resolveMedia(this.#client, p.thumbnail_url ?? p.thumbnail ?? '', 'image'), post_caption: p.post_caption ?? p.caption ?? '', likes_count: p.likes_count ?? p.like ?? 0, comments_count: p.comments_count ?? p.comment ?? 0, shares_count: p.shares_count ?? p.share ?? 0, post_url: p.post_url ?? p.url ?? '', post_deeplink: p.post_deeplink ?? p.deeplink ?? '', source_app: p.source_app || p.source || 'INSTAGRAM', footer_label: p.footer_label ?? p.footer ?? '', footer_icon: Toolkit.resolveMedia(this.#client, p.footer_icon ?? p.icon ?? '', 'image'), is_carousel: posts.length > 1, orientation: p.orientation ?? 'LANDSCAPE', post_type: p.post_type ?? 'VIDEO', __typename: 'GenAIPostPrimitive' }));
    ...
    async build({ forwarded = true, notification = false, includesUnifiedResponse = true, includesSubmessages = true, quoted, quotedParticipant, ...options } = {}) {
        ...
        return {
            ...
            ...this._extraPayload,
            botForwardedMessage: { message: { richResponseMessage: { messageType: 1, submessages: includesSubmessages ? await waitAllPromises(this._submessages) : [], unifiedResponse: { data: includesUnifiedResponse ? Buffer.from(JSON.stringify({ response_id: crypto.randomUUID(), sections })).toString('base64') : '' }, contextInfo: { ...forward, ...qObj, ...this._contextInfo } } } }
        };
    }
    ...
    static tokenizer(code, lang = 'javascript') {
        const keywordsMap = {
            javascript: new Set(['break','case','catch','continue','debugger','delete','do','else','finally','for','function','if','in','instanceof','new','return','switch','this','throw','try','typeof','var','void','while','true','false','null','undefined','class','const','let','super','extends','export','import','yield','static','constructor','async','await','get','set']),
            ...
            php: new Set(['abstract','and','array','as','break','callable','case','catch','class','clone','const','continue','declare','default','die','do','echo','else','elseif','empty','enddeclare','endfor','endforeach','endif','endswitch','endwhile','extends','final','finally','for','foreach','function','global','goto','if','implements','include','include_once','instanceof','interface','match','namespace','new','null','or','private','protected','public','require','require_once','return','static','switch','throw','trait','try','use','var','while','yield']),
        ...
        if (!lang || ['txt','text','plaintext'].includes(lang.toLowerCase())) {
